...
local drm_mode = require("libdrm.drm_mode")

--[[
	DRM_IOCTL_BASE			'd'
	DRM_IO(nr)			_IO(DRM_IOCTL_BASE,nr)
	DRM_IOR(nr,type)		_IOR(DRM_IOCTL_BASE,nr,type)
	DRM_IOW(nr,type)		_IOW(DRM_IOCTL_BASE,nr,type)
	DRM_IOWR(nr,type)		_IOWR(DRM_IOCTL_BASE,nr,type)
	...
	DRM_IOCTL_MODE_ATTACHMODE	DRM_IOWR(0xA8, struct drm_mode_mode_cmd) -- deprecated (never worked)
	DRM_IOCTL_MODE_DETACHMODE	DRM_IOWR(0xA9, struct drm_mode_mode_cmd) -- deprecated (never worked)
	...
	DRM_IOCTL_MODE_ATOMIC		DRM_IOWR(0xBC, struct drm_mode_atomic)
--]]
