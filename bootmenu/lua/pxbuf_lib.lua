...
function pxbuf_lib.new(w,h)
	...
	buf.scale_nearest = pxbuf_lib.scale_nearest
	buf.scale_pixelart_2x = pxbuf_lib.scale_pixelart_2x
...
function pxbuf_lib.copy_rect(origin_buf, target_buf, target_x,target_y, w,h, origin_x,origin_y)
	...
	-- use ffi.copy if we have compatible buffers
	if origin_buf.mem and target_buf.mem then
		...
		(origin_buf.w==target_buf.w) and (origin_buf.h==target_buf.h) then
			-- fastpath 1-1 complete copy
			ffi.copy(target_buf.mem, origin_buf.mem, origin_buf.len)
			return
		elseif (target_x==0) and (origin_x==0) and
		(origin_buf.w==w) and (target_buf.w==w) then
			-- fastpath for offset region copy
			-- TODO
		else
			-- use line-based copy
			-- TODO
...
function pxbuf_lib.scale_nearest(origin_buf, target_buf, target_x,target_y, origin_x,origin_y, origin_w,origin_h, scale_x,scale_y)
	target_x, target_y = tonumber(target_x) or 0, tonumber(target_y) or 0
	origin_w,origin_h = tonumber(origin_w) or origin_buf.w, tonumber(origin_h) or origin_buf.h
	origin_x, origin_y = tonumber(origin_x) or 0, tonumber(origin_y) or 0
	scale_x, scale_y = tonumber(scale_x) or 1, tonumber(scale_y) or 1

	local new_w = math.floor(origin_w*scale_x)
	local new_h = math.floor(origin_h*scale_y)

	for cy=0, new_h-1 do
		for cx=0, new_w-1 do
			local ox = math.floor(origin_x+(cx/(new_w-1))*origin_w+0.5)
			local oy = math.floor(origin_y+(cy/(new_h-1))*origin_h+0.5)
			local r,g,b,a = origin_buf.get_px(ox, oy)
			if r then
				target_buf.set_px(cx+target_x, cy+target_y, r,g,b,a)
			end
		end
...
-- copy a rectangular region from one pixel buffer to another, scale using a simple pixel scaling algorithm
function pxbuf_lib.scale_pixelart_2x(origin_buf, target_buf, target_x,target_y, origin_x,origin_y, origin_w,origin_h)
	target_x, target_y = tonumber(target_x) or 0, tonumber(target_y) or 0
	origin_w, origin_h = tonumber(origin_w) or origin_buf.w, tonumber(origin_h) or origin_buf.h
	origin_x, origin_y = tonumber(origin_x) or 0, tonumber(origin_y) or 0

	local function pack_pixel(r,g,b)
		return string.char(r or 0,g or 0,b or 0)
	end
	local function unpack_pixel(p)
		local r,g,b = p:byte(1,3)
		return r,g,b,255
	end

	for cy=0, origin_h-1 do
		for cx=0, origin_w-1 do
			local sp = pack_pixel(origin_buf.get_px(origin_x+cx, origin_y+cy))
			local sa = pack_pixel(origin_buf.get_px(origin_x+cx, origin_y+cy-1))
			local sb = pack_pixel(origin_buf.get_px(origin_x+cx+1, origin_y+cy))
			local sc = pack_pixel(origin_buf.get_px(origin_x+cx-1, origin_y+cy))
			local sd = pack_pixel(origin_buf.get_px(origin_x+cx, origin_y+cy+1))

			local t1,t2,t3,t4 = sp, sp, sp, sp
			if sc==sa and sc~=sd and sa~=sb then t1=sa end
			if sa==sb and sa~=sc and sb~=sd then t2=sb end
			if sd==sc and sd~=sb and sc~=sa then t3=sc end
			if sb==sd and sb~=sa and sd~=sc then t4=sd end

			target_buf.set_px(cx*2+target_x, cy*2+target_y, unpack_pixel(t1))
			target_buf.set_px(cx*2+target_x+1, cy*2+target_y, unpack_pixel(t2))
			target_buf.set_px(cx*2+target_x, cy*2+target_y+1, unpack_pixel(t3))
			target_buf.set_px(cx*2+target_x+1, cy*2+target_y+1, unpack_pixel(t4))
		end
	end
end
