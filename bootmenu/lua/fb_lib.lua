...
function fb_lib.new(fbdev, do_backbuffer)
	local fb = {}
	fb.dev = fbdev
	...
	local target_buf_u8 = mem_u8
	if do_backbuffer then
		--target_buf = ffi.new("uint8_t[?]", len)
		target_buf_u8 = ffi.gc(ffi.cast("uint8_t*", ffi.C.malloc(len)), ffi.C.free)
	end
	...
	function fb.flip()
		if do_backbuffer then
			ffi.copy(mem_u8, target_buf_u8, len)
		end
	end
	...
	function fb.dump()
		return ffi.string(target_buf_u8)
	end

	local function get_px_16bpp_rgb565(x,y)
		local i = y*vinfo.xres*2+x*2
		local byte_a, byte_b = target_buf_u8[i], target_buf_u8[i+1]
	...
	local function get_px_24bpp_rgb(x,y)
		...
		local r,g,b = target_buf_u8[i], target_buf_u8[i+1], target_buf_u8[i+2]
		return r,g,b
	end

	local function get_px_32bpp_rgba(x,y)
		local i = y*vinfo.xres*4+x*4
		local r,g,b,a = target_buf_u8[i], target_buf_u8[i+1], target_buf_u8[i+2], target_buf_u8[i+3]
		return r,g,b,a
	end
	...
	local function set_px_16bpp_rgb565(x,y,r,g,b)
		...
		target_buf_u8[i] = byte_a
		target_buf_u8[i+1] = byte_b
	end
